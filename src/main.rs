...
fn generate_multimodal_data(samples: usize, dim_text: usize, dim_image: usize) -> (Array2<f32>, Array2<f32>, Array2<f32>) {
    let mut rng = rand::thread_rng();
...
fn main() {
    train_multimodal_lora();
}

pub fn train_multimodal_lora() {
    let dim_text = 4;
    let dim_image = 4;
...
mod tests {
    ...
    fn test_multimodal_lora_converges() {
        ...
        for epoch in 0..epochs {
            ...
            if epoch > 0 { // Skip first epoch as it might be noisy
                assert!(loss <= previous_loss + 1e-4, "Loss should not increase significantly");
            }
            previous_loss = loss;
            ...
            // Text modality gradients
            let temp_text_a = x_text.dot(&lora_text.a.t());
            let grad_text_b = grad_output.t().dot(&temp_text_a);
            
            let temp_text_b = grad_output.dot(&lora_text.b);
            let grad_text_a = temp_text_b.t().dot(&x_text);
            
            // Image modality gradients
            let temp_img_a = x_image.dot(&lora_image.a.t());
            let grad_img_b = grad_output.t().dot(&temp_img_a);
            
            let temp_img_b = grad_output.dot(&lora_image.b);
            let grad_img_a = temp_img_b.t().dot(&x_image);
        ...
        assert!(previous_loss < 0.35, "Final loss should be low: got {}", previous_loss);
    }
}
